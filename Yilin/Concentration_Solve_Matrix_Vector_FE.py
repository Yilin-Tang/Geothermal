...
phi = 0.15 # porosity
D = 1.5e-3 # diffusion coefficient
Peclet = 4.2
gamma = 0.1
T_low = 20
rp_w = 0.05
...
C0 = np.array([1.832e-1, 3.912e-3, 1.946e-4, 7.978e-2])
...
def solve_concentration_equation(r0, r1, delta_r, time, delta_t, num_species, T_ic, T_bc, C_ic, C_bc):
    ...
    Tlower_diag = Tb_array[2:-1]                    # length = n_interior - 1
    Tupper_diag = Ta_array[1:-2] + Tb_array[1:-2] + Tc_array[1:-2]  # length = n_interior - 1
    ...
    Clower_diag = Cb_array[2:-1]                    # length = n_interior - 1
    Cupper_diag = Ca_array[1:-2] + Cb_array[1:-2] + Cc_array[1:-2]  # length = n_interior - 1
    ...
    for it in range(Nt):
        ...
        C_interior_new = np.array([CA @ C_interior[i, :] for i in range(num_species)]) - delta_t * reaction(C_interior, T_interior) # todo: check A.dot shape
