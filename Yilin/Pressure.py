...
def solve_pressure_equation(r0, r1, N, p0, dp0):
    ...
    for i in range(1, N):
        ri = r0 + i*h
        p[i + 1] = (2*ri + h)/(ri + h) * p[i] - ri/(ri + h) * p[i - 1]

    return r, p

r_w = 0.05 # radius of well
r_out = 5
...
# Solve and plot
r, p = solve_pressure_equation(r_w, r_out, N=10000, p0=p_w, dp0=dp_w)

c2 = 1/kappa * r_w * u_inj * np.log(r_w) + p_w
p_ex = - 1/kappa * r_w * u_inj * np.log(r) + c2

plt.plot(r, p, label='Numerical solution')
plt.plot(r, p_ex, label='Exact solution', linestyle='--')
plt.xlabel('r')
plt.ylabel('p(r)')
plt.title('Solution of Pressure')
plt.grid()
plt.legend()
plt.show()